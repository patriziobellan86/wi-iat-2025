...
def compute_accuracy_and_significance(system_outputs, correct_answers):
    ...
    for idx, entry in enumerate(system_outputs.values()):
        ...
        correct_answer_norm = normalize_answer(correct_answer, answers)

        # Final decision maker's answer
        system_answer = entry.get("final-decison-maker-answer", "").strip()
        system_answer_norm = normalize_answer(system_answer, answers)
...
def compute_metrics(system_outputs, correct_answers):
    total_count = len(system_outputs)
    if total_count == 0:
        return 0, 0, {}, {}, 0, 0, 0  # Return zeros for additional metrics
    ...
    for idx, entry in enumerate(system_outputs.values()):
        ...
        if correct_answer and system_answer:
            y_true.append(normalize_answer(correct_answer, answers))
            y_pred.append(normalize_answer(system_answer, answers))
...
def write_accuracies_to_csv(csv_file, results):
    ...
    with open(csv_file, 'w', newline='') as f:
        ...
        for res in results:
            ...
            row.append(f"{res['precision']:.2%}")
            row.append(f"{res['recall']:.2%}")
            row.append(f"{res['f1']:.2%}")
            writer.writerow(row)
...
def main():
    ...
    output_csv_file = "new_accuracy_socialsupport.csv"
    results = []
    ...
    for system_output_file, correct_answer_file in system_output_paths:
        ...
        precision, recall, f1 = compute_metrics(system_outputs, correct_answers)
